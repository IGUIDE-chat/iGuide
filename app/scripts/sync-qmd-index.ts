...
const QMD_HOME =
    ...
    (process.env.HOMEDRIVE && process.env.HOMEPATH
        ? `${process.env.HOMEDRIVE}${process.env.HOMEPATH}`
        : undefined);
const QMD_CACHE_HOME = process.env.XDG_CACHE_HOME || (QMD_HOME ? path.join(QMD_HOME, '.cache') : undefined);
...
function resolveQmdCli() {
    ...
    candidates.add(path.resolve(PROJECT_ROOT, 'node_modules/@tobilu/qmd/dist/cli/qmd.js'));

    if (process.env.APPDATA) {
        candidates.add(path.join(process.env.APPDATA, 'npm', 'node_modules', '@tobilu', 'qmd', 'dist', 'cli', 'qmd.js'));
    }

    try {
        const npmRootGlobal = runCapture(NPM_BIN, ['root', '-g']).trim();
        if (npmRootGlobal) {
            candidates.add(path.join(npmRootGlobal, '@tobilu', 'qmd', 'dist', 'cli', 'qmd.js'));
        }
    } catch {
...
function ensureCollection(spec: CollectionSpec) {
    ...
    if (!info.exists) {
        ...
        runQmd(['collection', 'add', expectedPath, '--name', spec.name, '--mask', DEFAULT_GLOB]);
        return;
    }

    const pathMismatch = !info.path || normalizePath(info.path) !== normalizePath(expectedPath);
