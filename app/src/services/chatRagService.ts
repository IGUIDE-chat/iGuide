...
const QUERY_EXPANSION_RULES: Array<{ pattern: RegExp; terms: string[] }> = [
  {
    pattern: /grainger|工程图书馆|工图/i,
    terms: ["Grainger Engineering Library"],
  },
  ...
  {
    pattern: /safe\s*walk|safe\s*ride|夜路|陪走|护送/i,
    terms: ["SafeWalks", "SafeRides"],
  },
  ...
  {
    pattern: /签证|国际生|i-20|opt|cpt|visa/i,
    terms: ["international student services"],
  },
  ...
  {
    pattern: /isr|伊利诺伊街/i,
    terms: ["Illinois Street Residence Hall", "ISR"],
  },
  ...
  {
    pattern: /par|宾州大道|宾夕法尼亚/i,
    terms: ["Pennsylvania Avenue Residence Hall", "PAR"],
  },
  {
    pattern: /far|佛州大道|佛罗里达大道/i,
    terms: ["Florida Avenue Residence Hall", "FAR"],
  },
...
function extractAsciiKeywords(query: string): string[] {
  ...
  return matches.filter((token) => {
    ...
    if (token.length === 2 && token === token.toLowerCase()) {
      return false
    }
    return true
  })
