...
export function isUiucOfficialUrl(url: string): boolean {
  try {
    ...
    return (
      hostname === UIUC_OFFICIAL_HOST ||
      hostname.endsWith(`.${UIUC_OFFICIAL_HOST}`)
    )
  } catch {
    return false
  }
...
export async function webSearchWithOfficialPriority(
  ...
  try {
    ...
    const hasEnoughOfficialResults =
      mergedOfficialResults.length >= Math.min(maxResults, 2)
    ...
    const fallbackResults = (
      await Promise.all(
        uniqueQueries.map((query) =>
          webSearch(query, { maxResults, searchDepth })
        )
      )
    ).flat()
