...
export const DormDetailInfo: React.FC<DormDetailInfoProps> = ({
  ...
  return (
    <motion.section variants={fadeUp} className="space-y-3 md:space-y-4">
      <h3 className="text-[16px] font-bold text-slate-900 md:text-[18px]">{t.amenities}</h3>
      <div className="flex flex-wrap gap-2 md:gap-2.5">
        {neutralTags.map((tag, i) => {
          ...
          return (
            ...
            >
              {Icon && <Icon className="size-3.5 text-slate-500 md:size-4" strokeWidth={1.5} />}
        ...
        {mutedTags.map((tag, i) => {
          ...
          return (
            ...
            >
              {Icon && <Icon className="size-3.5 text-slate-400 md:size-4" strokeWidth={1.5} />}
