                  ...
                  <div className="border-t-4 border-gray-100">
                    ...
                    <div className="border-b border-gray-100 px-4 py-3">
                      <div className="relative">
                        ...
                        <ChevronDown className="pointer-events-none absolute top-1/2 right-2.5 size-4 -translate-y-1/2 text-gray-400" />
                      </div>
                    </div>
                    <div className="divide-y divide-gray-100">
                      {planRows.map((row, ri) => {
    ...
    return (
      <div className="max-h-[calc(85vh-72px)] overflow-auto">
        <table className="w-full min-w-[760px] border-collapse">
          <thead>
            <tr className="bg-gray-50">
              <th className="sticky left-0 z-10 border-r border-b border-gray-200 bg-gray-50 px-4 py-3 text-left text-sm font-semibold text-gray-700">
                {t.feature}
              </th>
              {dorms.map((dorm) => {
                const dormName =
          ...
          <tbody>
            {comparisonRows.map((row, rowIndex) => (
              ...
              >
                <td className="sticky left-0 z-10 border-r border-b border-gray-200 bg-inherit p-4 text-sm font-medium text-gray-700">
                  <div className="flex items-center gap-2">
                    <span>{row.label}</span>
                    {row.highlightBest && (
                ...
                {dorms.map((dorm) => {
                  ...
                  return (
                    <td
                      ...
                      className={`border-b border-gray-200 p-4 align-top text-sm ${isBest ? "bg-emerald-50/60" : ""} `}
                    >
                      {row.getValue(dorm)}
                    </td>
                  )
        ...
        {dorms.some((d) => (dormPlans[d.id]?.length ?? 0) > 0) && (
          <div className="border-t-4 border-gray-100">
            <div className="border-b border-gray-200 bg-gray-50 p-4 md:px-6">
              ...
              {dorms.map((dorm) => {
                ...
                if (plans.length === 0) {
                  return (
                    <div
                      key={dorm.id}
                      className="flex items-center border-r border-gray-200 p-3 text-sm text-gray-400 italic last:border-r-0"
                    >
