/**
 * @file ./src/components/housing/hooks/useDormUserInteraction.ts
...
export const useDormUserInteraction = () => {
  ...
  const toggleFavorite = useCallback(
    async (dormId: string, dormName: string, dormNameZh?: string) => {
      if (user) {
        ...
        try {
          const result = await dormFavoritesService.toggleFavorite(
            dormId,
            dormName,
            dormNameZh
          );
  ...
  const addToHistory = useCallback(
    async (dorm: Dorm) => {
      ...
      if (user) {
        try {
          await dormViewingService.addToHistory(
            dorm.id,
            dorm.name,
            dorm.name_zh
          );
