...
const missingDorms = [
    { dormId: 'isr', url: 'https://www.google.com/maps/place/Illinois+Street+Residence+Halls/@40.1099,-88.2343,17z/' },
    { dormId: 'par', url: 'https://www.google.com/maps/place/Pennsylvania+Avenue+Residence+Halls/@40.1018,-88.2220,17z/' },
    { dormId: 'allen', url: 'https://www.google.com/maps/place/Allen+Residence+Hall/@40.1041,-88.2276,17z/' },
    { dormId: 'bousefield', url: 'https://www.google.com/maps/place/Bousfield+Hall/@40.1032,-88.2363,17z/' },
    { dormId: 'bromley', url: 'https://www.google.com/maps/place/Bromley+Hall/@40.1085,-88.2347,17z/' },
    { dormId: 'illini-tower', url: 'https://www.google.com/maps/place/Illini+Tower/@40.1081,-88.2315,17z/' },
    { dormId: 'newman', url: 'https://www.google.com/maps/place/Newman+Hall/@40.1052,-88.2314,17z/' },
    { dormId: 'hendrick', url: 'https://www.google.com/maps/place/Hendrick+House/@40.1088,-88.2202,17z/' },
    { dormId: 'presby', url: 'https://www.google.com/maps/place/Presby+Hall/@40.1074,-88.2337,17z/' },
    { dormId: 'armory', url: 'https://www.google.com/maps/place/Armory+House/@40.1050,-88.2332,17z/' },
];
...
async function scrapeDorm(page, dormId, mapUrl) {
    ...
    for (let tab of tabs) {
        ...
        if (text && (text.includes('Reviews') || text.includes('评价') || text.includes('评论'))) {
            await tab.click();
            clicked = true;
    ...
    if (!clicked) {
        console.log(`[${dormId}] WARNING: Could not find reviews tab. Trying to scrape visible reviews anyway...`);
    }
    ...
    const reviews = await page.evaluate(() => {
        ...
        if (results.length === 0) {
            ...
            for (const card of altCards) {
                const authorEl = card.querySelector('[class*="author"], [class*="name"], .TSUbDb a');
                const author = authorEl ? authorEl.innerText.trim() : 'Google User';

                const textEl = card.querySelector('[class*="review-text"], .Jtu6Td, .review-full-text');
                const text = textEl ? textEl.innerText.trim() : '';
                if (!text || text.length < 15) continue;

                const starEl = card.querySelector('[aria-label*="star"], [aria-label*="Star"]');
                let stars = 3;
                if (starEl) {
                    const label = starEl.getAttribute('aria-label') || '';
                    const match = label.match(/(\d)/);
    ...
    console.log(`[${dormId}] Page: "${pageTitle}" @ ${pageUrl.substring(0, 80)}...`);
...
async function main() {
    ...
    let fileContent = fs.readFileSync('src/components/housing/constants/googleReviews.ts', 'utf8');

    const lines = allCollected.map(r =>
    ...
    fs.writeFileSync('src/components/housing/constants/googleReviews.ts', fileContent, 'utf8');
