...
async function scrapeDorm(page, dormId, query) {
    ...
    for (let tab of tabs) {
        ...
        if (text && (text.includes('Reviews') || text.includes('评价') || text.includes('评论') || text.includes('Review'))) {
            await tab.click();
            clicked = true;
            break;
        }
    ...
    try {
        for (let i = 0; i < 5; i++) {
            ...
            if (element) {
                await page.evaluate((el) => { el.scrollBy(0, 10000); }, element);
            } else {
                 await page.evaluate(() => {
    ...
    for (const [dormId, query] of Object.entries(pchDorms)) {
        ...
        while (attempts < 2 && data.length === 0) {
            try {
                ...
                console.log(`[${dormId}] Error on attempt ${attempts + 1}: ${err.message}`);
            }
            attempts++;
        }
