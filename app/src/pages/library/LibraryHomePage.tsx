...
const LibraryHomePage: React.FC<LibraryHomePageProps> = ({ language }) => {
	...
	return (
		<div
			className="animate-fade-in-up no-scrollbar size-full min-w-0 overflow-y-auto"
		>
				...
				<div className="py-10 text-center">
					...
					<div className="relative mx-auto mb-16 max-w-xl">
						<input
							type="text"
							className="
         block w-full rounded-full border border-slate-200 bg-white px-5 py-3.5
         text-base text-slate-900 placeholder-slate-400 shadow-sm
       ...
       "
        ...
        "
						...
						{filteredArticles.length === 0 ? (
							<div
								className="
          rounded-2xl border border-dashed border-slate-200 bg-white/50 py-16
          text-center
        "
							>
								...
								{filteredArticles.map((article) => {
									...
									return (
										<div
											...
											className="
             group cursor-pointer rounded-3xl border border-slate-100 bg-white
             p-6 shadow-sm transition-all duration-300
             hover:-translate-y-1 hover:border-illini-blue/20 hover:shadow-xl
             hover:shadow-slate-200/50
           "
										>
											<span
												className="
              mb-4 inline-block rounded-full bg-slate-100 px-3 py-1 text-[10px]
              font-bold tracking-wider text-slate-500 uppercase
              transition-colors
              group-hover:bg-illini-blue/10 group-hover:text-illini-blue
            "
											>
												{categoryText?.label}
											</span>
											<h3
												className="
              mb-3 text-lg/tight font-bold text-slate-900 transition-colors
						...
						{CATEGORIES.map((category) => {
							...
							return (
								<div
									...
									className="
           group cursor-pointer rounded-2xl border border-slate-100 bg-white p-7
           shadow-sm transition-all duration-300
           hover:-translate-y-1 hover:shadow-md hover:shadow-slate-200/50
         "
								>
									<div
										className="
            mb-6 flex size-14 items-center justify-center rounded-2xl
            bg-slate-50 text-3xl shadow-inner transition-colors duration-300
