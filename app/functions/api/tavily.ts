...
export const onRequestPost: PagesFunction<Env> = async (context) => {
    ...
    try {
        ...
        if (!apiKey) {
            return new Response(
                JSON.stringify({ error: 'Missing TAVILY_API_KEY in server environment.' }),
        ...
        if (!tavilyRes.ok) {
            ...
            return new Response(
                JSON.stringify({ error: 'Tavily API request failed.', status: tavilyRes.status, details: errText }),
