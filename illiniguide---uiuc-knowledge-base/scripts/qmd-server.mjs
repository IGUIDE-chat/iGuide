...
const EMBEDDING_MODEL = process.env.EMBEDDING_MODEL || 'Xenova/multilingual-e5-small';
...
function collectMdFiles(dir) {
  ...
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory() && entry.name !== '.qmd' && entry.name !== 'node_modules') {
...
async function initVectorSearch() {
  try {
    ...
    vectorReady = true;
    console.log(`[Vector] Ready. ${docEmbeddings.length} document embeddings loaded.`);
  } catch (err) {
...
async function buildDocEmbeddings() {
  ...
  for (const fullPath of mdFiles) {
    ...
    computed++;
    if (computed % 10 === 0) console.log(`[Vector] Embedded ${computed} documents...`);
  }

  if (computed > 0) {
    console.log(`[Vector] Computed ${computed} new embeddings, saving to disk...`);
...
function matchesLang(filePath, lang) {
  ...
  let norm = filePath.replace(/\\/g, '/').replace(/^qmd:\/\/[^/]+\//, '').toLowerCase();
...
function rrfFuse(listA, listB, k = 60) {
  ...
  for (const [rank, item] of listA.entries()) {
    ...
    const s = 1 / (k + rank + 1);
    if (prev) { prev.score += s; } else { scores.set(id, { score: s, item: { ...item, file: id } }); }
  }
...
async function queryQmd({ query, lang = 'en', limit = 10, mode = 'fusion' }) {
  ...
  const safeMode = ['bm25', 'vector', 'fusion'].includes(mode) ? mode : 'fusion';
  const candidateLimit = Math.max(safeLimit * 3, safeLimit);
  const trimmedQuery = query.trim();

  // Check cache
  const key = makeCacheKey({ query, lang: safeLang, mode: safeMode, limit: safeLimit });
  ...
  try {
    ...
    if (safeMode === 'fusion') {
      ...
      const [bm25Raw, vecRaw] = await Promise.all([
        runBm25Search(trimmedQuery, candidateLimit)
          .then((r) => r.filter((item) => matchesLang(item.file, safeLang))),
      ...
      results = raw.filter((item) => matchesLang(item.file, safeLang)).slice(0, safeLimit);
    }
...
const server = createServer(async (req, res) => {
  ...
  if (req.url === '/api/cache/clear' && req.method === 'POST') {
    if (!authOk) { sendJson(res, 401, { error: 'Unauthorized' }); return; }
  ...
  if (req.url === '/api/embeddings/rebuild' && req.method === 'POST') {
    if (!authOk) { sendJson(res, 401, { error: 'Unauthorized' }); return; }
    if (!embedder) { sendJson(res, 503, { error: 'Embedding model not loaded' }); return; }
    try {
      await buildDocEmbeddings();
      cacheClear();
  ...
  if (req.url === '/api/search' && req.method === 'POST') {
    ...
    try {
      ...
      console.log(`[QMD] q="${(body.query || '').slice(0, 50)}" mode=${body.mode || 'fusion'} lang=${body.lang || 'en'} n=${results.length} ${elapsed}ms ${elapsed < 5 ? 'cache' : ''}`);
      sendJson(res, 200, results);
    } catch (err) {
      if (err.message === 'queue_full') {
        sendJson(res, 503, { error: 'Server overloaded, try again later' }, { 'Retry-After': '5' });
      } else if (err.message === 'queue_timeout') {
        sendJson(res, 504, { error: 'Request queued too long, try again later' });
      } else {
...
// ── Startup ─────────────────────────────────────────────────────
console.log(`[QMD Server] CLI: ${QMD_CLI} (${QMD_IS_SCRIPT ? 'script' : 'module'})`);
console.log(`[QMD Server] Content: ${QMD_CONTENT}`);
console.log(`[QMD Server] Region: ${REGION} | Port: ${PORT} | Auth: ${API_KEY ? 'on' : 'off'}`);
console.log(`[QMD Server] Concurrency: max ${MAX_CONCURRENT}, queue ${MAX_QUEUE}`);
