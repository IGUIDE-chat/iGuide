...
import type { Dorm, BathroomScope } from '../src/components/housing/types/index';
...
const HANDBOOK_OCR_SCRIPT = path.resolve(PROJECT_ROOT, 'scripts/generate-handbook-ocr.py');
...
function getArticleContent(article: Article, lang: 'en' | 'zh') {
  return lang === 'zh'
    ? sanitizeParagraph(article.content_zh) || sanitizeParagraph(article.content)
    : sanitizeParagraph(article.content);
}

function getArticleTags(article: Article, lang: 'en' | 'zh') {
  return lang === 'zh' && article.tags_zh?.length ? article.tags_zh : article.tags;
}
...
function getDormTitle(dorm: Dorm, lang: 'en' | 'zh') {
  return lang === 'zh'
    ? sanitizeLine(dorm.name_zh) || dorm.name
    : dorm.name;
}

function getDormDescription(dorm: Dorm, lang: 'en' | 'zh') {
  return lang === 'zh'
    ? sanitizeParagraph(dorm.description_zh) || sanitizeParagraph(dorm.description)
...
function getHousingTypeLabel(housingType: Dorm['housingType'], lang: 'en' | 'zh') {
  if (lang === 'zh') {
    return housingType === 'URH' ? '大学宿舍（URH）' : '认证私营宿舍（PCH）';
  }
  return housingType === 'URH' ? 'University Residence Halls (URH)' : 'Private Certified Housing (PCH)';
}
...
function buildRoomOptionLine(option: NonNullable<Dorm['roomOptions']>[number], lang: 'en' | 'zh') {
  const parts: string[] = [];
  ...
  if (option.bedCount != null) {
    parts.push(lang === 'zh' ? `${option.bedCount}人` : `${option.bedCount} bed`);
  }
  if (option.bathroomScope === 'communal' || option.bathroomScope === 'individual-use') {
    ...
    parts.push(lang === 'zh' ? `${option.bathroomCount}卫` : `${option.bathroomCount} bath`);
  } else {
    parts.push(getBathroomScopeLabel(option.bathroomScope, lang));
  }
...
function buildFloorPlanLine(plan: NonNullable<Dorm['floorPlans']>[number], lang: 'en' | 'zh') {
  const parts: string[] = [];
  const displayName = sanitizeLine(plan.officialName) || plan.labelCode || plan.type || (lang === 'zh' ? '未命名房型' : 'Unnamed layout');
...
function buildDormMarkdown(record: DormDocumentRecord, lang: 'en' | 'zh') {
  ...
  const quickFacts = [
    renderOptionalValue(lang === 'zh' ? '英文名' : 'English Name', lang === 'zh' ? dorm.name : sanitizeLine(dorm.name_zh)),
    `- ${lang === 'zh' ? '住宿类型' : 'Housing Type'}: ${getHousingTypeLabel(dorm.housingType, lang)}`,
    `- ${lang === 'zh' ? '位置' : 'Location'}: ${getDormLocation(dorm, lang)}`,
    renderOptionalValue(lang === 'zh' ? '年费用' : 'Annual Price', formatCurrency(dorm.price)),
    renderOptionalValue(lang === 'zh' ? '申请费' : 'Application Fee', formatCurrency(dorm.applicationFee)),
    ...
    renderOptionalValue(lang === 'zh' ? '地址' : 'Address', lang === 'zh' ? sanitizeLine(dorm.address_zh) || dorm.address : dorm.address),
  ...
  const roomOptions = dorm.roomOptions?.length
    ? dorm.roomOptions.map((option) => `- ${buildRoomOptionLine(option, lang)}`).join('\n')
    : `- ${lang === 'zh' ? '无房型选项数据' : 'No room option data'}`;

  const floorPlans = dorm.floorPlans?.length
    ? dorm.floorPlans.map((plan) => `- ${buildFloorPlanLine(plan, lang)}`).join('\n')
  ...
  return frontmatter + [
    ...
    `## ${lang === 'zh' ? '优点' : 'Pros'}`,
    '',
    renderBulletList(getDormPros(dorm, lang), lang === 'zh' ? '暂无优点信息' : 'No pros provided'),
    '',
    `## ${lang === 'zh' ? '注意点' : 'Cons'}`,
    '',
    renderBulletList(getDormCons(dorm, lang), lang === 'zh' ? '暂无注意点信息' : 'No cons provided'),
    '',
    `## ${lang === 'zh' ? '标签' : 'Tags'}`,
    '',
    `- ${dorm.tags.join(', ')}`,
    ...
    '',
    sanitizeParagraph(dorm.diningNearbyDetail) || (lang === 'zh' ? '无额外餐饮信息。' : 'No additional dining details.'),
    '',
...
function mapDormRow(row: Record<string, unknown>): DormDocumentRecord {
  ...
  const dorm = finalizeDormRecord(normalizeDorm({
    id: row.id as string,
    name: (row.name as string) ?? fallbackDorm?.name ?? '',
    name_zh: (row.name_zh as string) ?? fallbackDorm?.name_zh,
    description: (row.description as string) ?? fallbackDorm?.description ?? '',
    description_zh: (row.description_zh as string) ?? fallbackDorm?.description_zh,
    imageUrl: (row.image_url as string) ?? fallbackDorm?.imageUrl ?? '',
    price: Number(row.price) || fallbackDorm?.price || 0,
    priceRange: (row.price_range as Dorm['priceRange']) ?? fallbackDorm?.priceRange ?? '$',
    location: (row.location as string) ?? fallbackDorm?.location ?? 'Main Quad',
    location_zh: (row.location_zh as string) ?? fallbackDorm?.location_zh,
    housingType: (row.housing_type as Dorm['housingType']) ?? fallbackDorm?.housingType ?? 'URH',
    ac: row.ac != null ? Boolean(row.ac) : (fallbackDorm?.ac ?? false),
    dining: (row.dining as Dorm['dining']) ?? fallbackDorm?.dining ?? 'nearby',
    diningNearbyDetail: (row.dining_nearby_detail as string) ?? fallbackDorm?.diningNearbyDetail,
    bathroomType: (row.bathroom_type as Dorm['bathroomType']) ?? fallbackDorm?.bathroomType ?? 'communal',
    lat: Number(row.lat) || fallbackDorm?.lat || 0,
    lng: Number(row.lng) || fallbackDorm?.lng || 0,
    tags: (row.tags as string[]) ?? fallbackDorm?.tags ?? [],
    structuredTags: (row.structured_tags as Dorm['structuredTags']) ?? fallbackDorm?.structuredTags,
    categorizedTags: (row.categorized_tags as Dorm['categorizedTags']) ?? {
      ...
      ...(fallbackDorm?.categorizedTags.llcNames?.length ? { llcNames: fallbackDorm.categorizedTags.llcNames } : {}),
    },
    roomTypes: (row.room_types as Dorm['roomTypes']) ?? fallbackDorm?.roomTypes ?? [],
    roomOptions: (row.room_options as Dorm['roomOptions']) ?? fallbackDorm?.roomOptions,
    floorPlans: sanitizeFloorPlansForStorage(row.floor_plans as Dorm['floorPlans']) ?? fallbackDorm?.floorPlans,
    galleryImages: (row.gallery_images as string[]) ?? fallbackDorm?.galleryImages,
    ...
    applicationFee: row.application_fee != null ? Number(row.application_fee) : fallbackDorm?.applicationFee,
...
async function fetchDormRecords(): Promise<DormDocumentRecord[]> {
  ...
  if (!supabaseUrl || !supabaseKey) {
    console.warn('[qmd] Supabase env missing. Falling back to static dorm data.');
  ...
  try {
    ...
    if (error) {
      console.warn(`[qmd] Failed to fetch dorms from Supabase: ${error.message}`);
      return fallbackDormRecords();
    }

    if (!data?.length) {
      console.warn('[qmd] Supabase dorm query returned no rows. Falling back to static dorm data.');
      return fallbackDormRecords();
    }

    return data.map((row) => mapDormRow(row as Record<string, unknown>));
  } catch (error) {
    console.warn('[qmd] Supabase dorm fetch threw. Falling back to static dorm data.', error);
    return fallbackDormRecords();
  }
}
...
async function generateArticleCorpus() {
  for (const lang of ['en', 'zh'] as const) {
    ...
    for (const article of ARTICLES) {
      await writeFile(path.join(outputDir, `${article.id}.md`), buildArticleMarkdown(article, lang));
    }
  }
...
async function generateDormCorpus() {
  ...
  for (const lang of ['en', 'zh'] as const) {
    ...
    for (const record of records) {
      await writeFile(path.join(outputDir, `${record.dorm.id}.md`), buildDormMarkdown(record, lang));
    }
  }
...
function generateHandbookCorpus() {
  const outputPath = path.join(QMD_ROOT, 'handbook', 'zh', 'uiuc-new-student-handbook-2025.md');
