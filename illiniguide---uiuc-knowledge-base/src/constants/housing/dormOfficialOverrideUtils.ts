import { Dorm, DormCategorizedTags, FloorPlan } from '../../types/housing';

export type DormOverride = Partial<
    Omit<Dorm, 'structuredTags' | 'categorizedTags' | 'floorPlans' | 'galleryImages'>
> & {
...
export const urhPage = (slug: string) => `https://www.housing.illinois.edu/living-communities/halls/${slug}`;
...
function normalizeFloorPlanMedia(planValue: FloorPlan): FloorPlan {
    return {
        ...planValue,
        imageUrl: planValue.imageUrl ? normalizeOfficialMediaUrl(planValue.imageUrl) : undefined,
        photoUrl: planValue.photoUrl ? normalizeOfficialMediaUrl(planValue.photoUrl) : undefined,
...
export function normalizeOverrideMedia<T extends DormOverride>(override: T): T {
    return {
        ...override,
        imageUrl: override.imageUrl ? normalizeOfficialMediaUrl(override.imageUrl) : override.imageUrl,
...
export const urls = (...values: string[]) => values.map(normalizeOfficialMediaUrl);
export const plan = (value: FloorPlan): FloorPlan => ({ available: true, ...value });
