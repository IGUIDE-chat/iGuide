...
export function normalizeCategorizedTags(categorizedTags: DormCategorizedTags): DormCategorizedTags {
...
function syncStructuredTags(
    ...
    return {
        ...structuredTags,
        laundry: structuredTags.laundry || categorizedTags.facilities.includes('laundry'),
...
export function sanitizeFloorPlanForStorage(plan: FloorPlan): FloorPlan {
    ...
    const normalizedPhotoUrls = sanitizeUrlList([...(photoUrls ?? []), legacyPhotoUrl]);
...
export function sanitizeFloorPlansForStorage(floorPlans?: FloorPlan[] | null): FloorPlan[] | undefined {
