                ...
                <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-lg border border-gray-100 py-1 z-50 animate-in fade-in zoom-in-95 duration-100 origin-top-right">
                    {SORT_OPTIONS.map((option) => (
                        <button
                            ...
                            type="button"
                            className="group w-full flex items-center justify-between px-4 py-2 text-sm text-left hover:bg-illini-blue/5 transition-colors first:rounded-t-lg last:rounded-b-lg"
                        >
                            <span className={sortBy === option.value ? 'font-medium text-illini-blue' : 'text-gray-600 group-hover:text-illini-blue/80'}>
                                {option.label}
                            </span>
                            {sortBy === option.value && <Check size={14} className="text-illini-blue" />}
                        </button>
                    ))}
                </div>
            )}
