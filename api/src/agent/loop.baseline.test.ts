...
test("hi message blocks retrieval tools", async () => {
	...
	try {
		...
		const requestBody = parseRequestBody(mockFetch.requests[0]);
		assert.deepEqual(requestBody.tools, [], "Hi message should have empty tools array");
	} finally {
		globalThis.fetch = originalFetch;
	}
});
...
test("good morning message blocks retrieval tools", async () => {
	...
	try {
		...
		const requestBody = parseRequestBody(mockFetch.requests[0]);
		assert.deepEqual(requestBody.tools, [], "Good morning should have empty tools array");
	} finally {
		globalThis.fetch = originalFetch;
	}
});
...
test("provider error triggers fallback", async () => {
	...
	try {
		...
		await assert.rejects(
			...
			(error: Error) => {
				assert.ok(error.message.includes("500") || error.message.includes("error"));
				return true;
			},
