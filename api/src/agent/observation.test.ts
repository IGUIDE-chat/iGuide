...
test("buildObservation converts successful tool results into canonical observations", () => {
	const observation = buildObservation({
		...
		input: { query: "PAR" },
		result: { content: "PAR has multiple dining options.\nSource: housing guide" },
		stepIndex: 2,
	});
	...
	assert.equal(observation.output, "PAR has multiple dining options.\nSource: housing guide");
	assert.equal(observation.raw, "PAR has multiple dining options.\nSource: housing guide");
...
test("buildObservation preserves error result diagnostics", () => {
	...
	assert.equal(observation.raw, '{"error":"execution_failed","tool":"grep_docs","message":"permission denied"}');
