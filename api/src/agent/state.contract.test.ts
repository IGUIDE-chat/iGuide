...
test("LoopState has required fields", () => {
	...
	assert.ok(Array.isArray(state.currentToolCalls), "currentToolCalls must be an array");
	assert.equal(state.currentToolCalls.length, 0, "Initial currentToolCalls should be empty");

	assert.ok(Array.isArray(state.observations), "observations must be an array");
	assert.equal(state.observations.length, 0, "Initial observations should be empty");
...
test("LoopState tracks iteration count separately from step index", () => {
	...
	assert.equal(typeof state.iterationCount, "number", "iterationCount must be a number");
...
test("advanceStep increments step index", () => {
	...
	assert.equal(nextState.stepIndex, 1, "After one advance, stepIndex should be 1");

	const finalState = advanceStep(nextState);
	assert.equal(finalState.stepIndex, 2, "After two advances, stepIndex should be 2");
});
...
test("state after advance is not first turn", () => {
	...
	assert.equal(isFirstTurn(nextState), false, "After advance should not be first turn");
});
...
test("replay turn has prior observations", () => {
	...
	assert.equal(isReplayTurn(state), true, "State with prior observations should be replay");
});
