...
test('toolCallId and toolName are separate fields', () => {
  ...
  assert.notEqual(
    obs.toolCallId,
    obs.toolName,
    'toolCallId and toolName must be distinct'
  )
})
...
test('Observation has error field that is null on success', () => {
  ...
  assert.equal(
    successObs.error,
    null,
    'Successful observation should have null error'
  )
})
...
test('Observation error field contains error details on failure', () => {
  ...
  assert.ok(
    errorObs.error !== null,
    'Error observation should have non-null error'
  )
...
test('truncated observation still has valid output', () => {
  ...
  assert.ok(
    obs.output !== null,
    'Truncated observation should still have output'
  )
