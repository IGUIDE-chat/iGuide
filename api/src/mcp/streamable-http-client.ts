...
export class StreamableHttpMCPClient implements MCPAdapterClient {
		...
		try {
			...
			if (
				...
				return {
					success: false,
					failure_reason: "invalid_mcp_response",
					error_message: "MCP response did not match the expected JSON-RPC envelope",
					latency_ms,
					raw_response: parsedResponse.value,
				};
			}
			...
			return {
				...
				error_message: error instanceof Error ? error.message : "MCP endpoint was unreachable",
				latency_ms: null,
			};
		} finally {
		...
		if (!Array.isArray(result.tools)) {
			return {
				...
				result: {
					...
					failure_reason: "invalid_mcp_response",
					tools: [],
					error_message: "MCP tools/list response did not contain a tools array",
				},
			};
		}
	...
	private mapToolResult(
		...
		| { success: false; failure_reason: MCPFailureReason; error_message: string } {
...
function formatRpcError(error: unknown): string {
	...
	const codePart = typeof rpcError.code === "number" ? ` (${rpcError.code})` : "";
	const messagePart =
...
function formatToolContent(content: unknown, fallback: Record<string, unknown>): string {
