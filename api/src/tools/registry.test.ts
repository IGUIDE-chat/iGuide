...
import { createStubTool, createEchoTool, createFailingTool } from "../test/utils/stubTools.ts";
import type { RequestContext, ToolDefinition, ToolResult } from "./types.ts";

const MOCK_CTX: RequestContext = { env: {}, userId: "test-user", region: "global" };

test("execute returns tool result on success", async () => {
	const registry = new ToolRegistry();
	registry.register(createEchoTool());

	const result = await registry.execute("stub_echo", { query: "hello" }, MOCK_CTX);
...
test("truncation handles maxResultBytes smaller than suffix", async () => {
	...
	registry.register(createStubTool({ name: "tiny_tool", content: "x".repeat(100) }));
...
test("execute handles unicode content", async () => {
	...
	registry.register(createStubTool({ name: "unicode_tool", content: unicodeContent }));
...
test("truncation with unicode counts bytes correctly", async () => {
	...
	registry.register(createStubTool({ name: "unicode_trunc", content: unicodeContent }));
...
test("execute handles very large content", async () => {
	...
	registry.register(createStubTool({ name: "huge_tool", content: largeContent }));
