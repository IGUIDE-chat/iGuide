...
interface Env {
    ...
    QMD_API_URL_GLOBAL: string; // Chicago VPS QMD daemon
    QMD_API_URL_CN: string; // Alibaba Cloud QMD daemon
    QMD_API_KEY?: string; // Shared secret for QMD auth
}
...
export default {
    async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
        ...
        try {
            ...
            // Search endpoint - proxy to QMD daemon (geo-routed)
            if (pathname === '/search' || pathname === '/api/search') {
                ...
                // Select QMD node based on geo-IP
                const qmdUrl = isCN
                    ? (env.QMD_API_URL_CN || env.QMD_API_URL_GLOBAL)
                    : env.QMD_API_URL_GLOBAL;
                ...
                const collection = body.lang === 'zh' ? 'uiuc-zh' : 'uiuc-en';

                const qmdHeaders: Record<string, string> = {
                    'Content-Type': 'application/json',
                };
                if (env.QMD_API_KEY) {
                    qmdHeaders['Authorization'] = `Bearer ${env.QMD_API_KEY}`;
                }

                const qmdResponse = await fetch(`${qmdUrl}/query`, {
                    method: 'POST',
                    headers: qmdHeaders,
                    body: JSON.stringify({
                        query: body.query,
                ...
                const searchData = await qmdResponse.text();
                return new Response(searchData, {
                    status: qmdResponse.status,
                    headers: {
                        ...corsHeaders,
                        'Content-Type': 'application/json',
                        'X-QMD-Region': isCN ? 'cn' : 'global',
                    },
                });
            }
            ...
            return new Response(
                JSON.stringify({
                    error: 'Not found',
                    path: pathname,
                    availableEndpoints: ['/health', '/chat', '/search'],
                }),
                {
                    status: 404,
                    headers: {
